package com.xiaodou.control.prop;
 ...
 * @version 1.0
 */
public class DingDingProp {
  /**
   * 配置文件
   */
  private static FileUtil errFile = FileUtil
      .getInstance("/conf/custom/env/dingding_message.properties");
  ...
  private static FileUtil getProperty() {
    if (errFile == null)
      synchronized (DingDingProp.class) {
        if (errFile == null)
          errFile = FileUtil.getInstance("/conf/custom/env/dingding_message.properties");
      }
    return errFile;
  }
