...
public class HttpHelper {
  ...
  private static String apiDocPath;
      ...
      for (SubFieldEntity key : keys) {
        ...
        if (key.getType().startsWith("List") || key.getType().startsWith("Set")
          ...
          if (value instanceof ApiClassJavadocEntity) {
            ...
            String jarPath =
                JavadocService.basePath + "/target/" + JavadocService.projectName + ".jar";
            String libPath =
                JavadocService.basePath + "/target/" + JavadocService.projectName + "/WEB-INF/lib";
            String classPath = realValue.getFullName();
            Object initJson = ClassLoaderHelper.getInitJSONObject(jarPath, classPath, libPath);
            String initJsonStr = JSON.toJSONString(initJson);
            subJson += initJsonStr;
          }
          subJson += "]";
        } else {
  ...
  public static void setPath(String projectName) {
    apiDocPath = JavadocService.basePath + String.format("/target/%s/apidocs", projectName);
  }
}
