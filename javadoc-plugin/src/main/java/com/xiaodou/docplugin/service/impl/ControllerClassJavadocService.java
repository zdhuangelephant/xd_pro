 ...
 *
 */
public class ControllerClassJavadocService implements IClassJavadocService {
	...
	public ApiClassJavadocEntity getClassJavadoc(File javaFile, String fullName) {
		...
		String requestMapping = getRequestMapping(type);
		String realReqMapping = "";
		if(requestMapping.equals("")){
			realReqMapping = "";
		}else {
			realReqMapping = requestMapping.startsWith("/") ? requestMapping : "/" + requestMapping;
		}
		classJavadocEntity.setRequestMapping(realReqMapping);
		//获取类描述与作者
		getDescription(type);
