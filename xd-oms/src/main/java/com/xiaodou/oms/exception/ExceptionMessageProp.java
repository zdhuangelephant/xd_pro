...
public class ExceptionMessageProp {
   ...
   * 配置文件
   */
  private static FileUtil errFile = FileUtil.getInstance("/conf/custom/notenv/error_info.properties");

  /**
   * @return 获取配置文件信息
   */
  private static FileUtil getProperty() {
    if (errFile == null) synchronized (ExceptionMessageProp.class) {
      if (errFile == null) errFile = FileUtil.getInstance("/conf/custom/notenv/error_info.properties");
    }
    return errFile;
  }
