function main(password, salt, vcode, isMd5){
    salt = strToJs(salt);
    return Encryption.getEncryption(password, salt, vcode);
}
...
function test(password, salt, vcode,isMd5) {
    salt = strTojs(salt);
    var md5Pwd = isMd5 ? password : Encryption.md5(password);
    var    h1 = hexchar2bin(md5Pwd);

    var strTobytes = TEA.strToBytes(salt);

    var md = Encryption.md5(salt);

    return salt +" "+h1 + " " + strTobytes +" " + md;
}
...
RSA = function () {
    ...
    if (Z && (appName == "Microsoft Internet Explorer")) {
        ar.prototype.am = ay;
        aw = 30
    } else {
        if (Z && (appName != "Netscape")) {
            ar.prototype.am = b;
            aw = 26
        } else {
            ar.prototype.am = ax;
