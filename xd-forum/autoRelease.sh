...
function downloadModule(){
	...
	download_version=$3
	third_version=`echo $download_version | awk -F '.' {'print $3'}`
	if [ $third_version -gt 0 ]; then
		newUrl=$newUrl$artifactId"Branches/"$artifactId"-"$download_version
	else
		newUrl=$newUrl$artifactId"/Trunk"
	fi
	normal_echo "==>start download project from $newUrl ..."
	svn checkout $newUrl $artifactId-$download_version
	downloadResult=$?
	
	if [ $downloadResult -ne 0 ]; then
		if [ $third_version -gt 0 ]; then
			newUrl="http://svn.elong.cn/svn/Code/HotelMIS/"$artifactId"/Branches/"$artifactId"-"$download_version
		else
	...
	if [ $downloadResult -ne 0 ]; then
		if [ $third_version -gt 0 ]; then
			newUrl="http://svn.elong.cn/svn/Code/ElongOrder/"$artifactId"/Branches/"$artifactId"-"$download_version
		else
	...
	if [ $downloadResult -ne 0 ]; then
		if [ $third_version -gt 0 ]; then
			newUrl="http://svn.elong.cn/svn/Code/Pub/"$artifactId"/Branches/"$artifactId"-"$download_version
		else
	...
	if [ $downloadResult -ne 0 ]; then
		if [ $third_version -gt 0 ]; then
			newUrl="http://svn.elong.cn/svn/Code/zhilian/"$artifactId"/Branches/"$artifactId"-"$download_version
		else
	...
	if [ $downloadResult -ne 0 ]; then
		if [ $third_version -gt 0 ]; then
			newUrl="http://svn.elong.cn/svn/Code/JHotelMIS/"$artifactId"/Branches/"$artifactId"-"$download_version
		else
...
if [ ! -d $artifactId ]; then
	downloadModule $artifactId $url $version
...
if [ -e snapshotsMessage.log ]; then
	...
	do
		...
		if [ $flag -eq 0 ]; then
		  ...
		  releaseSnapshot $artifactId $scmUrl $version
		  if [ $? -eq 0 ]; then
