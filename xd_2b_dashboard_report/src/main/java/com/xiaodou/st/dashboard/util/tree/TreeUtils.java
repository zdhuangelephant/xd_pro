package com.xiaodou.st.dashboard.util.tree;
...
public class TreeUtils {
   ...
   * @return
   */
  public String getTree(String parentId, String templateStr, String selectedId, String adds,
      String templateStrGroup) {
    if (adds == null) {
      adds = "";
    }
    // 返回的结果
    ...
    if (childs.size() > 0) {
      ...
      for (TreeNode node : childs) {
        ...
        if (number.equals(size)) {
          ...
          sign = icons[1];
          // span用│符号
          span = StringUtils.isNotBlank(adds) ? icons[0] : "";
        }
        // 空格串
        ...
        result =
            result
                + this.getTree(tempId, templateStr, selectedId, adds + span + nbsp,
                    templateStrGroup);
        number = number + 1;
      }
    }
   ...
   * @return
   */
  public String getTreeMulti(String parentId, String templateStr, List<String> selectedIds,
      String adds, String templateStrGroup) {
    if (adds == null) {
      adds = "";
    }
    // 返回的结果
    ...
    if (childs.size() > 0) {
      ...
      for (TreeNode node : childs) {
        ...
        result =
            result
                + this.getTreeMulti(tempId, templateStr, selectedIds, adds + span + nbsp,
                    templateStrGroup);
        number = number + 1;
      }
    }
   ...
   * @return
   */
  private String getJqueryTree(String parentId, String treeId, String parentTemplateStr,
      String childTemplateStr, String treeClass, Boolean isTop) {
    String result = "";
    // 获取所有子节点
    List<TreeNode> childs = this.getChild(parentId);
    // 看是否为top节点
    ...
    for (TreeNode treeNode : childs) {
      ...
      try {
        if (nodes.size() <= 0) {
          ...
          result =
              result
                  + this.getJqueryTree(treeNode.getId(), treeId, parentTemplateStr,
                      childTemplateStr, treeClass, false);
        }
      } catch (Exception e) {
        e.printStackTrace();
        return "error";
   ...
   * @return
   */
  public String getJqueryTree(String parentId, String treeId, String parentTemplateStr,
      String childTemplateStr, String treeClass) {
    return this.getJqueryTree(parentId, treeId, parentTemplateStr, childTemplateStr, treeClass,
        true);
  }
   ...
   * @return
   */
  public String getJqueryTableTree(String parentId, String parentTemplateStr,
      String childTemplateStr) {
    StringBuilder result = new StringBuilder("");
