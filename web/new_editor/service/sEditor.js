...
save.saveChapter = function (cinfo, callback) {
    if (!cinfo) return;
    var sql = "insert into xd_editor_chapter (`id`, `name`, `prefix`, `level`) values (?, ?, ?, ?)";
    var args = [cinfo.id, cinfo.name, cinfo.prefix, cinfo.level];
    db.execQuery({
        sql: sql, args: args, callback: function (results) {
            if (results) {
                callback("ok");
...
update.updateCourseware = function (cinfo, callback) {
    ...
    cinfo._download_url = mpath.join(cinfo.currpath, _downfile);
    saveFile(cinfo, function (saveedit, savedownload) {
        var sql = "update xd_editor_courseware set `name` = ?, `filename` = ?, `dfilename` = ?, `edit_url` = ?, `show_url` = ?, `download_url` = ? where `id` = ?";
        var args = [cinfo.name, cinfo.file, _downfile, 'output/' + _editfile, '/courseware/show?id=' + cinfo.id, 'output/' + _downfile, cinfo.id];
        db.execQuery({
            sql: sql, args: args, callback: function (results) {
                if (results) {
                    callback(_editfile);
...
update.updateChapter = function (cinfo, callback) {
    if (!cinfo) return;
    var sql = "update xd_editor_chapter set `name` = ? where `id` = ?";
    var args = [cinfo.name, cinfo.id];
    db.execQuery({
        sql: sql, args: args, callback: function (results) {
            if (results) {
                callback("ok");
...
update.recoverChapter = function (cinfo, callback) {
    if (!cinfo) return;
    var sql = "update xd_editor_chapter set `status` = 1 where `prefix` = ? and `name` = ?";
    var args = [cinfo.prefix, cinfo.name];
    db.execQuery({
        sql: sql, args: args, callback: function (results) {
            if (results) {
...
saveOrUpdate.chapter = function (cinfo, callback) {
    if (!cinfo) return;
    var sql = "select * from xd_editor_chapter where `id` = ? or (`name` = ? and `prefix` = ?)";
    var args = [cinfo.id, cinfo.name, cinfo.prefix];
    db.execQuery({
        sql: sql, args: args, callback: function (results) {
            if (results && results.length > 0) {
                if (results[0].id == cinfo.id) {
                    update.updateChapter(cinfo, callback);
                } else {
                ...
                save.saveChapter(cinfo, callback);
            }
        }
    });
}
...
saveOrUpdate.courseware = function (cinfo, callback) {
    if (!cinfo) return;
    var sql = "select * from xd_editor_courseware where `id` = ? or (`name` = ? and `prefix` = ?)";
    var args = [cinfo.id, cinfo.name, cinfo.prefix];
    db.execQuery({
        sql: sql, args: args, callback: function (results) {
            if (results && results.length > 0) {
...
var recover = {};
recover.chapter = function (cinfo, callback) {
    if (!cinfo) return;
    var sql = "select * from xd_editor_chapter where `name` = ? and `prefix` = ? and status = 0";
    var args = [cinfo.name, cinfo.prefix];
    db.execQuery({
        sql: sql, args: args, callback: function (results) {
            if (results && results.length > 0) {
                update.recoverChapter(cinfo, callback);
            } else {
                callback("Error:章节不存在.")
            }
        }
    });
}
