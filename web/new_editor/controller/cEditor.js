...
function savaOrUpdate(req, res, next, action) {
    var level = req.body.level ? req.body.level : null;
    if (level && parseInt(level) < 5) {
        savaOrUpdateChapter(req, res, next, action.chapter);
    } else {
...
function savaOrUpdateChapter(req, res, next, action) {
    var params = {};
    params.prefix = req.body.prefix ? req.body.prefix : null;
    params.id = req.body.id ? req.body.id : null;
    params.name = req.body.name ? req.body.name : null;
    params.level = req.body.level ? req.body.level : null;
    ...
    if (!checkInfo.isOk) {
        ...
        action(params, function (_filename) {
            ...
            if (_filename) {
                if (_filename == "Err:same") {
                    ret.retcode = -1;
                    ret.retdesc = "存在同名章节,请修改章节名.";
                } else if (_filename == "Err:deleted") {
                    ret.retcode = -3;
                    ret.retdesc = "该章节已被删除,是否需要恢复?";
                } else {
                    ret.retcode = 0;
                    ret.retdesc = "保存成功";
                }
