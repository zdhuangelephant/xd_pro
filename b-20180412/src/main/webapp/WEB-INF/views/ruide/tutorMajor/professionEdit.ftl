...
<form name="form1" id="editForm" method="post" class="form-horizontal" role="form"
	action="/tutorMajor/profession/doEdit">
	<input type="hidden" name="type" value="${profession.type}" />
	<input type="hidden" name="id" value="${profession.id}" />
	<div class="form-group">
		<label class="col-sm-3 control-label no-padding-right"
			for="form-field-1"> 作者 </label>
		<div class="col-sm-9">
			<input type="text" datatype="s" sucmsg="haha"
				value="${profession.author}" id="name" name="author" id="form-field-2"
	...
	<div class="form-group">
		...
		<div class="col-sm-9">
			<input type="text" datatype="s" sucmsg="haha"
				value="${profession.majorName}" id="name" name="majorName" id="form-field-2"
				placeholder="" class="col-xs-10 col-sm-5" />
		</div>
	</div>
	<div class="form-group">
        <label class="col-sm-3 control-label no-padding-right" for="form-field-1">专业类别</label>
        <div class="col-sm-9">
            <select id="majorId" name="majorCategoryId" class="form-control">
           		<#list majorCategoryList as majorCategory>
               		<option value="${majorCategory.id}" <#if profession.majorCategoryId == majorCategory.id>selected</#if> >${majorCategory.majorCategory}</option>
           		</#list>
       		</select>
        </div>
    </div>
	<div class="form-group">
		...
		<div class="col-sm-9">
			<img class="portrait1" src="${profession.image}" alt="专业配图"> 
			<input
				type="hidden" datatype="s" sucmsg="haha" value="${profession.image}"
				id="pic1" name="image" placeholder="" class="col-xs-10 col-sm-5" />
			<a onclick="uploadProtrait1()">上传</a>
		</div>
		<br />
		<div class="col-sm-9" style="color: red">*建议上传265*173像素的jpg、png、gif图片</div>
	</div>


	<div class="form-group">
		<label class="col-sm-3 control-label no-padding-right"
			for="form-field-3"> 标题</label>
		<div class="col-sm-9">
			<input type="text" datatype="s" sucmsg="haha"
				value="${profession.title}" id="form-field-3" name="title"
				placeholder="" class="col-xs-10 col-sm-5" />
		</div>
	</div>
	<div class="form-group">
		<label class="col-sm-3 control-label no-padding-right"
			for="form-field-3"> 副标题</label>
		<div class="col-sm-9">
			<input type="text" datatype="s" sucmsg="haha"
				value="${profession.subtitle}" id="form-field-3" name="subtitle"
				placeholder="" class="col-xs-10 col-sm-5" />
		</div>
	</div>
	<div class="form-group">
		...
		<div class="col-sm-9">
			<img class="portrait2" src="${profession.contentImage}" alt="正文图片">
			<input type="hidden" datatype="s" sucmsg="haha"
				value="${profession.contentImage}" id="pic2" name="contentImage" placeholder=""
	...
	<div class="operationArea form-group">
		<ul class="choose">
	...
	<div id="tempStore" style="display: none;">${profession.content}</div>
	</div>
	<script>
		$(function() {
