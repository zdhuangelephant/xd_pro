...
$(function() {
	$("#addBtn").bind('click',function() {
				...
				var author = check("author", "author", "请输入作者.");
				if (!author)
					return;
				var portrait = $("#pic1").val();
				if (!portrait)
					return;
				var majorId = check("majorId", "majorId", "请选择相关专业.");
				if (!majorId)
					return;
				var publishTime = check("publishTime", "publishTime", "请选择发布时间.");
				if (!publishTime)
					return;
				var thinkDesc = check("thinkDesc", "thinkDesc", "请填写感悟简介.");
				if (!thinkDesc)
					return;
				var content = $("#storeContent").html();
				var contentImage = $("#pic2").val();
				
				
				$.ajax({
					url : baseurl + '/student/doAdd',
					data : {
						'author' : author,
						'portrait' : portrait,
						'majorId' : majorId,
