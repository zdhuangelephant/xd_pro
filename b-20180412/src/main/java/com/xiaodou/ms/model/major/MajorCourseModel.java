...
public class MajorCourseModel {
  ...
  public void initMajorCourseInfo(String majorCourseInfo) {
    if (StringUtils.isJsonNotBlank(majorCourseInfo)) {
      ...
      if (null != mInfo) {
        ...
        if (StringUtils.isNotBlank(mInfo.getExamDate())) examDate = mInfo.getExamDate();
        if (StringUtils.isNotBlank(mInfo.getCredit())) credit = mInfo.getCredit();
        if (StringUtils.isNotBlank(mInfo.getCourseType())) courseType = mInfo.getCourseType();
        if (StringUtils.isNotBlank(mInfo.getMode())) mode = mInfo.getMode();
        if (mInfo.getCreateTime() != null) createTime = mInfo.getCreateTime();
        if (StringUtils.isNotBlank(mInfo.getExamDateType())) examDateType = mInfo.getExamDateType();
      }
    }
  }
  ...
  public static void main(String[] args) {
    MybatisXmlTool
        .getInstance(MajorCourseModel.class, "xd_major_course", "src/main/resources/conf/mybatis")
        .buildXml();
  }
