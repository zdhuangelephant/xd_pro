package com.xiaodou.common.util.consistentHash.pool;
...
public class ConsistentHashPool<T> {
   ...
   * @param node 节点
   */
  public void add(String nodename, NodeMap<T> node) {
    Integer numberOfReplicas =
        node.getNumberOfReplicas() > 0 ? node.getNumberOfReplicas() : this.numberOfReplicas;
    for (int i = 0; i < numberOfReplicas; i++) {
      circle.put(hashFunction.hashString(nodename + i, charset).asInt(), node.getNode());
    }
  }
