...
import com.xiaodou.mooccrawler.dao.course.CourseChapterDao;
import com.xiaodou.mooccrawler.domain.course.CourseChapterModel;
import com.xiaodou.mooccrawler.util.TreeNode;
import com.xiaodou.mooccrawler.util.TreeUtils;
import com.xiaodou.mooccrawler.web.request.ChapterCreateRequest;
import com.xiaodou.mooccrawler.web.request.ChapterEditRequest;
...
public class CourseChapterService {
   ...
   * @return
   */
  public String multiSelectChapterTree(List<String> ids, Integer subjectId) {
    List<CourseChapterModel> courseChapters = this.queryAllChildChapter(subjectId, 0);
   ...
   * @return
   */
  public CourseChapterModel findChapterById(Integer chapterId) {
    CourseChapterModel entity = new CourseChapterModel();
    entity.setId(chapterId);
    return courseChapterDao.findEntityById(entity);
  }

  public String ChapterTableTree(Integer courseId, Integer parentId, String template) {
   ...
   * @return
   */
  public String ChapterTableTree(Integer courseId, Integer parentId) {
    String template =
        "<tr>"
            + "<td>${node.id}</td>"
   ...
   * @return
   */
  public String ChapterJqueryTree(Integer courseId, Integer parentId, String parentTemplate,
   ...
   * @return
   */
  public String ChapterJqueryTree(Integer courseId, Integer parentId) {
   ...
   * @return
   */
  public String ChapterJqueryTreeCheckBox(Integer courseId, Integer parentId) {
    if (courseId==null||courseId==0){
      return "";
    }
    List<CourseChapterModel> ChapterList = this.queryAllChildChapter(courseId, parentId);
   ...
   * @return
   */
  public String ChapterSelectTree(Integer courseId, Integer parentId, String selectedId) {
    List<CourseChapterModel> ChapterList = this.queryAllChildChapter(courseId, parentId);
    List<TreeNode> treeNodeList = new ArrayList<>();
    for (CourseChapterModel chapterModel : ChapterList) {
      TreeNode treeNode = new TreeNode();
    ...
    String result =
        treeUtils.getTree(parentId.toString(),
            "<option value=\"${node.id}\" ${selected}>${spacer}${node.data.sindex}[${node.name}]</option>", selectedId,
            null, null);
    return result;
  }
   ...
   * @return
   */
  public List<CourseChapterModel> queryAllChildChapter(Integer courseId, Integer chapterId) {
    if (chapterId == 0) {
  ...
  public List<CourseChapterModel> queryChapterByIds(List<Integer> ids) {
    ...
    Page<CourseChapterModel> ChapterList =
        courseChapterDao.queryListByCondWithOutPage(cond, output);
   ...
   * @return
   */
  public List<CourseChapterModel> queryAllChapter(Integer courseId) {
    Map<String, Object> cond = new HashMap<String, Object>();
    cond.put("subjectId", courseId);
    Map<String, Object> output = new HashMap<>();
    output.put("id", "");
   ...
   * @return
   */
  private List<CourseChapterModel> parentList(Map<Integer, CourseChapterModel> idMap, Integer catId) {
   ...
   * @return
   */
  private List<CourseChapterModel> allChildId(Map<Integer, List<CourseChapterModel>> parentIdMap,
      Integer catId) {
    List<CourseChapterModel> result = new ArrayList<>();
  ...
  public List<CourseChapterModel> queryAllChildChapterNoChapter(Integer courseId, Integer chapterId) {
    ...
    }else return this.findChapterMissionById(chapterId);
  ...
  public List<CourseChapterModel> queryAllItemNoChapter(Integer courseId) {
    ...
    Page<CourseChapterModel> ChapterList =
        courseChapterDao.cascadeQueryChapter(cond, output);
    return ChapterList.getResult();
  }
  /**
   * 根据主键查询外联任务表
