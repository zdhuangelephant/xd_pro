...
<head>
<meta charset="UTF-8">
<title>${activity.title }_线下活动_瑞德在线</title>
<meta name="description" content="${activity.htmlContent}">
...
<body>
	...
	<div class="content-wrap">
		<div class="content">
			<div class="w1002">
				<div class="cont-left">
				<div class="bread">当前位置：<a href="/">首页</a>>><a href="/xxhd">线下活动</a>>><span>${activity.title }</span></div>
				<#if activity??>
					<div class="detail-content">
						<h1>${activity.title }</h1>
						<div class="detail-info detail-info2">
							<p>
								<span>活动时间：${activity.activityTime?string("yyyy-MM-dd") }
								</span><strong>活动地点：${activity.activityPlace }</strong>
							</p>
							<p>
								<span>截止报名：${activity.deadlineTime?string("yyyy-MM-dd") }</span><strong>导
									师：${activity.tutorMajorModel.author }</strong>
							</p>
						</div>
						<#if (activity.contentImage)??> 
							<div class="detail-imgage">
								<img src="${activity.contentImage }" alt="${activity.title }"> 
							</div>
						</#if> 
						<div class="detail-img">${activity.content }</div>
						<p class="zan">
							<span>阅读（${activity.readQuantity }）</span> <span> <a
								id="thumbsId" href="javascript:void(0)"
								onclick="doThumbs('${activity.id}','${activity.thumbNums}', '2')">赞(${activity.thumbNums})</a></span>
						</p>
					</div>
					<#else>
