...
public class RabbitConnectPool implements DisposeListener {
  ...
  public synchronized void setConfirmCallback(ConfirmCallback callback) {
    if (this.confirmCallback == null || this.confirmCallback == callback) { // 暂时仅实现支持一个callback
      ...
      throw new InvalidParameterException("Only one confirm callback supported.");
    }
  }
  ...
  public RabbitSendProxy getSendingConnection(boolean forceNew) throws AmqpClientException {
    if (!forceNew) {
      ...
      if (proxy != null) {
        ...
        throw new AmqpClientException("Unexpected proxy creation");
      }
    } else {
      // 强制创建一个新的连接
      ...
      if (this._max <= 0) {
        ...
        synchronized (lock) {
          try {
            ...
            LoggerUtil.error("return proxy to pool error", e);
          } finally {
            lock.notify();
          }
        }
   ...
   * @throws AmqpClientException
   */
  public RabbitReceiveProxy getReceiveConnection(int qos) throws AmqpClientException {
  ...
  private RabbitProxy getNewProxy(String functionName, int qos) throws AmqpClientException {
    for (int i = 0; i < 3; i++) {
      ...
      try {
        ...
        LoggerUtil.error("sleep for waitingTime error", e);
      }
    }
