...
def _execute(command):
     try:
    	...
    	c.stop(container=containerId)
    	time.sleep(10)
    	log = c.logs(container=containerId)
    	output = base64.encodestring(log)
    	state = '1'
    	param = ["{\"id\":\"" + command["id"] + "\",\"state\":\"" + state + "\",\"mac\":\"" + MachineUtil.get_mac_address() + "\",\"msg\":\"" + "123" +"\",\"version\":\"" + command["version"] + "\"}"]
        ...
        output = base64.encodestring(log)
        state = '1'
        param = ["{\"id\":\"" + command["id"] + "\",\"state\":\"" + state + "\",\"mac\":\"" + MachineUtil.get_mac_address() + "\",\"msg\":\"" + "error" +"\",\"version\":\"" + command["version"] + "\"}"]
        AosServerService.route_service("ThriftApiService", "editState", param) 
