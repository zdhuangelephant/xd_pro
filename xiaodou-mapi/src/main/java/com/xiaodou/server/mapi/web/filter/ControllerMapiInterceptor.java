...
import com.xiaodou.server.mapi.enums.MAINTAIN;
import com.xiaodou.summer.util.ReqInfoWrapper;
import com.xiaodou.summer.validator.IValidator;
import com.xiaodou.summer.vo.out.ResultInfo;
import com.xiaodou.summer.vo.out.ResultType;

public class ControllerMapiInterceptor {

  private String maintain_code = "015";
  ...
  public Object service(JoinPoint joinPoint) {
    ...
    if (null != maintain) {
      ...
      response.setRetcode(maintain.getCode());
      response.setRetdesc(maintain.getMsg());
      return FastJsonUtil.toJson(response);
    }
    Object resultInfo = service0(joinPoint);
    return resultInfo;
  }

  private MAINTAIN maintain() {
    CommonInfo commonInfo =
        CommonInfoUtil.getCommonInfoInfoByCode(maintain_code);
    if (null == commonInfo) return null;
    String maintainCode = commonInfo.getInfoVersion();
    if (StringUtils.isNotBlank(maintainCode)) {
      for (MAINTAIN maintain : MAINTAIN.values()) {
        if (maintain.getCode().equals(maintainCode)) return maintain;
        // ...
      }
    }
    return null;
  ...
  public Object batch(JoinPoint joinPoint) {
    ...
    try {
      ...
      action.setTraceId(TraceWrapper.getWrapper().getTraceParam().getTraceId());
  ...
  public Object service0(JoinPoint joinPoint) {
    long startTime = System.currentTimeMillis();
    Object object = null;
    Boolean hasError = false;
    String errorMessage = StringUtils.EMPTY;
    ...
    try {
      ...
      if (isCache.get()) {
        object =
            cacheService.getCacheInfo(outIn.getTargetClass(), outIn.getTargetMethod(),
                outIn.getParams().toString());
        // 从缓存获取失败,则执行逻辑并写入缓存
        if (null == object) {
          object = ((ProceedingJoinPoint) joinPoint).proceed();
          cacheService.cacheInfo(outIn.getTargetClass(), outIn.getTargetMethod(),
              outIn.getParams().toString(), object);
        }
        return object;
      }
      object = ((ProceedingJoinPoint) joinPoint).proceed();
      ...
      action.setTraceId(TraceWrapper.getWrapper().getTraceParam().getTraceId());
      outIn.setResponseInfo(getLimitString(result, 500));
      LoggerUtil.actionInfo(action);
