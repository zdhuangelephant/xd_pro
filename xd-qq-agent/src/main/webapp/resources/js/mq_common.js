    ...
    sys.onload(function () {
        ...
        window.setTimeout(function () {
            ...
            if (!window.g_login_sig) {
                pt.nlog("鏃х増鐧诲綍妗唋ogin_sig涓虹┖|_|" + window.g_pt_version, "291551")
            }
        }, 1000);
        try {
            ...
            g.innerHTML = '<input type="button" class="btn_close" id="close" name="close" onclick="javascript:onPageClose();" title="鍏抽棴" /><u id="label_login_title">鐢ㄦ埛鐧诲綍</u>';
            if (window.g_href && location.href.indexOf(g_href) == -1 && /^ui.ptlogin2./.test(location.hostname)) {
                if (location.protocol == "http:") {
                    if (window.g_jumpname == "aqjump" && i != 5 && r != 3 && r != 2) {
                        ...
                        v.innerHTML = '<div style="position:relative;">							<br/>							<p style="line-height:20px;text-align:left;width:220px;margin:0 auto;">鎮ㄥ綋鍓嶇殑缃戠粶瀛樺湪閾捐矾灞傚姭鎸侊紝涓轰簡纭繚鎮ㄧ殑甯愬彿瀹夊叏锛岃浣跨敤瀹夊叏鐧诲綍銆�</p></div>							<input id="safe_login" value="瀹夊叏鐧诲綍"" type="button" class="btn" style="text-align:center;"/>							</div>							<div style="margin-top:10px;margin-left:10px; height:20px;">							<span style="float:left;height:15px;width:14px; background: url(https://ui.ptlogin2.qq.com/style/14/images/help.png) no-repeat scroll right center transparent;"></span>							<a style="float:left; margin-left:5px;" href="http://kf.qq.com/info/80861.html" target="_blank" >浠€涔堟槸閾捐矾灞傚姭鎸�</a>							</div>';
                        ...
                        u.src = "http://badjs.qq.com/cgi-bin/js_report?bid=110&msg=" + s + "&v=" + Math.random()
                    } else {
                        var u = new Image();
                        var s = encodeURIComponent(window.g_href + "|_|" + location.href + "|_|" + window.g_jumpname + "|_|mid=245580");
                        u.src = "http://badjs.qq.com/cgi-bin/js_report?bid=110&mid=245580&msg=" + s + "&v=" + Math.random()
                    }
                } else {
...
function ptui_changeImg(f, d, i) {
    ...
    var b = (pt.isHttps ? "https://ssl." : "http://") + "captcha." + f + "/getimage?&uin=" + h + "&aid=" + d + "&" + Math.random();
    var a = $("imgVerify");
    try {
        if (a != null) {
            a.src = b;
...
function loadVC(a) {
    ...
    if (a == true) {
        ...
        var f = (pt.isHttps ? "https://ssl." : "http://") + "captcha." + g_domain + "/getimage?aid=" + g_appid + "&r=" + Math.random() + "&uin=" + g;
        var d = new Date();
        b.src = f;
...
if (_ua.indexOf("Android") > -1 || _ua.indexOf("iPhone") > -1 || _ua.indexOf("iPad") > -1 || _ua.indexOf("iPod") > -1) {
    $$(".input_login_text")[0].innerHTML = "鎵嬫満QQ 缃戦〉鐗�";
