...
function configCaseItem(caseItem) {
    ...
    var paramJson = caseItem.queryParam;  //type is json
    var temp = {};
    if(caseItem.mode=="application/x-www-form-urlencoded"){
        temp.mode = "urlencoded";
        temp.urlencoded = [];
        for (var prop in paramJson) {
            if (paramJson.hasOwnProperty(prop)) {
                ...
                temp.urlencoded.push(param);
            }
        }
    }else if(caseItem.mode=="application/json"){
        temp.mode = caseItem.mode
        temp.urlencoded = getQueryParamWithJson(paramJson);
    }else{
        throw new Error("不支持的mime类型");
    }
