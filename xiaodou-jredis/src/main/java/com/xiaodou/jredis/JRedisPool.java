...
public class JRedisPool {
  ...
  public JRedisPool(final String host) {
    this(new GenericObjectPoolConfig(), host, Protocol.DEFAULT_TIMEOUT, null,
        Protocol.DEFAULT_DATABASE, true);
  }

  public JRedisPool(final GenericObjectPoolConfig poolConfig, final String host, int timeout,
      final String password) {
    this(poolConfig, host, timeout, password, Protocol.DEFAULT_DATABASE, true);
  }

  public JRedisPool(final GenericObjectPoolConfig poolConfig, final String host, final int timeout) {
    this(poolConfig, host, timeout, null, Protocol.DEFAULT_DATABASE, true);
  }

  public JRedisPool(final GenericObjectPoolConfig poolConfig, final String host, int timeout,
      final String password, final int database, final boolean randomRead) {
    ...
    new HAConfig(poolConfig, infos);
    routeManager = new RouteManager(infos);
    jedis =
        (JedisCommands) Proxy.newProxyInstance(JedisCommands.class.getClassLoader(),
