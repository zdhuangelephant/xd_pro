...
import com.xiaodou.summer.source.jdbc.transaction.SummerTransactionManager;
...
public class SummerSqlSessionFactory implements SqlSessionFactory {
  ...
  private Map<String, SqlSessionFactory> otherSqlSessionFactorys = Maps.newHashMap();

  private String environment = SummerSqlSessionFactory.class.getSimpleName();
   ...
   * {@code SqlSessionFactory} configuration at runtime.
   * 
   ...
   * {@code DataSource} should match the one used by the {@code SqlSessionFactory}: for example, you
   * could specify the same JNDI DataSource for both.
   * 
   ...
   * the underlying target {@code DataSource}. If there's nevertheless a
   ...
   * any attempt at getting an SqlSession through Spring's MyBatis framework will throw an exception
   * if a transaction is active.
   * 
   * @see SpringManagedTransactionFactory
   * @param transactionFactory the MyBatis TransactionFactory
   */
  public void setTransactionFactoryManager(SummerTransactionManager transactionFactoryManager) {
  ...
  public void afterPropertiesSet() throws Exception {
    Assert.notNull(dataSourceManager, "Property 'dataSourceManager' is required");
    Assert.notNull(sqlSessionFactoryBuilder, "Property 'sqlSessionFactoryBuilder' is required");
    buildSqlSessionFactory();
  }

  @SuppressWarnings("unchecked")
  private Configuration buildSqlSessionFactory(DynamicDataSource defaultDataSource,
      TransactionFactory transactionFactory) throws IOException, SQLException {
    Configuration configuration;

    XMLConfigBuilder xmlConfigBuilder = null;
    if (this.configLocation != null) {
      xmlConfigBuilder =
          new XMLConfigBuilder(this.configLocation.getInputStream(), null,
    ...
    if (this.transactionFactoryManager == null) {
      transactionFactoryManager = new SummerTransactionManager(this.dataSourceManager);
    }
      ...
      for (Resource mapperLocation : this.mapperLocations) {
        ...
        try {
          XMLMapperBuilder xmlMapperBuilder =
              new XMLMapperBuilder(mapperLocation.getInputStream(), configuration, path,
    ...
    Environment menvironment =
        new Environment(this.environment, transactionFactory, defaultDataSource);
    configuration.setEnvironment(menvironment);
  ...
  protected void buildSqlSessionFactory() throws IOException, SQLException {
    ...
    if (null != this.dataSourceManager.getDefaultDataSource()) {
      Configuration configuration =
          buildSqlSessionFactory(this.dataSourceManager.getDefaultDataSource(),
              this.transactionFactoryManager.getDefaultTransactionFactory());
      this.defaultSqlSessionFactory = this.sqlSessionFactoryBuilder.build(configuration);
    }
    if (!CollectionUtils.isEmpty(this.dataSourceManager.getOtherDataSources())) {
      if (CollectionUtils.isEmpty(this.otherSqlSessionFactorys)) {
        for (Map.Entry<String, DynamicDataSource> entry : dataSourceManager.getOtherDataSources()
            .entrySet()) {
          Configuration configuration =
              buildSqlSessionFactory(entry.getValue(), this.transactionFactoryManager
                  .getOthertransactionFactory().get(entry.getKey()));
          this.otherSqlSessionFactorys.put(entry.getKey(),
   ...
   * {@inheritDoc}
   */
  public SqlSessionFactory getDefaultObject() throws Exception {
   ...
   * {@inheritDoc}
   */
  public Map<String, SqlSessionFactory> getOthersObject() throws Exception {
    if (this.otherSqlSessionFactorys == null) {
      afterPropertiesSet();
    }

    return this.otherSqlSessionFactorys;
  }
  ...
  public Class<? extends SqlSessionFactory> getObjectType() {
    return this.defaultSqlSessionFactory == null
        ? SqlSessionFactory.class
        : this.defaultSqlSessionFactory.getClass();
  }
  ...
  public void onApplicationEvent(ApplicationEvent event) {
    if (failFast && event instanceof ContextRefreshedEvent) {
      ...
      this.defaultSqlSessionFactory.getConfiguration().getMappedStatementNames();
      for (String key : this.otherSqlSessionFactorys.keySet()) {
        this.otherSqlSessionFactorys.get(key).getConfiguration().getMappedStatementNames();
      }
    }
  }
