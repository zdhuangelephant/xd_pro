...
@HandlerMessage("ChangeWrongRecord")
public class ChangeWrongQuesWorker extends AbstractDefaultWorker {
  ...
  private void updateWrongRecord(UserWrongRecord wrongRecord, UserWrongRecord oldRecord) {
    ...
    oldRecord.setWrongStatus(wrongRecord.getWrongStatus());
    if (QuesBaseConstant.QUES_WRONG_RECORD_STATUS_RESOLVED.equals(oldRecord.getWrongStatus())
        && StringUtils.isAllNotBlank(oldRecord.getModule(), oldRecord.getMajorId())) {
