...
import com.xiaodou.thrift.TransferParam;
...
public class RouteServiceImpl extends ScanPath implements RouteService.Iface {
  ...
  public DefaultModel routeService(String className, String funName, String paramValue)
      throws TException {
    OutInEntity msg = new OutInEntity();
    msg.setParams(paramValue);
    msg.setTargetClass("服务端接收到调用了！！！thrift--" + className);
    msg.setTargetMethod(funName);
    ...
    try {
      ...
      msg.setUseTime(System.currentTimeMillis() - useTime);
      String changeResult =
          ThriftUtil.changeResult(methd.invoke(impl, argments), methd.getReturnType());
      msg.setResponseInfo(changeResult);
      LoggerUtil.outInInfo(msg);
      return new DefaultModel(changeResult);
